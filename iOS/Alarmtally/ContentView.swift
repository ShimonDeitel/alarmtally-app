...
struct ContentView: View {
    ...
    @State private var editingEntry: AlarmtallyEntry?
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("Log how many times you hit snooze each morning.")
    ...
    private func entryRow(_ entry: AlarmtallyEntry) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.date.formatted(date: .abbreviated, time: .shortened))
                .font(Theme.captionFont)
                .foregroundStyle(Theme.textSecondary)
            HStack {
                Text("Snooze Count: \(entry.value1)")
                    .font(Theme.bodyFont)
                    .foregroundStyle(Theme.textPrimary)
                Spacer()
...
struct EntryFormView: View {
    ...
    private let existing: AlarmtallyEntry?
    let onSave: (AlarmtallyEntry) -> Void

    init(entry: AlarmtallyEntry?, onSave: @escaping (AlarmtallyEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            Form {
                Section("Morning") {
                    ...
                    TextField("Snooze Count", text: $value1)
